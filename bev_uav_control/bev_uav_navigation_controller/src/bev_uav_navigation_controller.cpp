#include "bev_uav_navigation_controller/navigation_controller.hpp"
...
Navigation_Controller::Navigation_Controller():nh("~"){
    ...
    subCollisionDetected    = nh.subscribe(ns + "/collisionAvoidance/emergencyStop",    1, &Navigation_Controller::cbCollisionBrake,    this);
...
void Navigation_Controller::Run(){
    ...
    while(ros::ok()){
        ...
        if(pc->setpointRec || vc->setpointRec || flagBypass){
            if((currentState.mode != "GUIDED") && (currentState.mode != "LAND") && (currentState.mode != "RTL") && !offboardRequest){
        ...
        if( ( !pc->setpointRec && !vc->setpointRec && !flagBypass) && flagWaypointReceived && !waypointReachedSkip){
            ROS_INFO("Waypoint finished!");
            pubWaypointReached.publish(msgEmpty);
            if((currentState.mode != "LAND") && (currentState.mode != "RTL")){
                pubPosHold.publish(msgEmpty);
                currentState.mode = "BRAKE";
            }
...
void Navigation_Controller::cbCollisionBrake(const std_msgs::Empty&){
    pc->setpointRec = false;
    vc->setpointRec = false;
    flagBypass = false;
    flagWaypointReceived = false;
    currentState.mode = "BRAKE";
}
...
void Navigation_Controller::cbState(const mavros_msgs::State::ConstPtr msg){
    if(currentState.mode == "GUIDED" && msg->mode != "GUIDED"){
        pc->setpointRec = false;
        vc->setpointRec = false;
        flagBypass = false;
        flagWaypointReceived = false;
        //        ROS_WARN("STOP THE NAVIGATION CONTROLLER");
    }
    currentState = *msg;
}
...
void Navigation_Controller::cbSetpoint(const bev_uav_msgs::UAVWaypoint &msg){
    ...
    switch(msg.type){
        ...
        case UAVWaypoint::TYPE_POSITION_YAW_FIRST_WP:
            ...
            pc->setpointRec = true;        // Make sure position control is ON

            vc->setpointRec = false;       // Make sure velocity control is OFF
        ...
        case UAVWaypoint::TYPE_VELOCITY_YAW_FIRST_WP:
            ...
            pc->flagWaypointReceived = true;  // Deactivate dummy message to position control
            pc->setpointRec = false;       // Make sure position control is OFF

            vc->setpointRec = true;        // Make sure velocity control is ON
