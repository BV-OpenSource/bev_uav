...
float Position_Controller::calculateWaypointHeading(geometry_msgs::PoseStamped currPos,bev_uav_msgs::UAVWaypoint goalPos){
    double errorY = goalPos.y - currPos.pose.position.y;
    double errorX = goalPos.x - currPos.pose.position.x;
    float heading = static_cast<float>(errorY*errorY + errorX*errorX < goalPoint.radius*goalPoint.radius?
