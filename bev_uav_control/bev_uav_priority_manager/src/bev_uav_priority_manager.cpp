...
Priority_manager::Priority_manager():n("~") {
    ...
    // Publishers
    pubSetpoint = n.advertise <bev_uav_msgs::UAVWaypoint> (ns + paramPubTopicSetpoint, 1);
...
void Priority_manager::cbHeifuMsgsHeifuWaypointmsg(const ros::MessageEvent<bev_uav_msgs::UAVWaypoint const>& event, const std::string& topic)
{
    ROS_DEBUG_STREAM("PUBLISHER " << event.getPublisherName());

    const bev_uav_msgs::UAVWaypointConstPtr& msg = event.getMessage();

    ROS_DEBUG_STREAM("TOPIC: " << topic.c_str());
    ROS_DEBUG_STREAM("XYZ message: " << msg->x << " " << msg->y << " " << msg->z);
...
void Priority_manager::cbGeometryMsgsPoseStampedmsg(const ros::MessageEvent<const geometry_msgs::PoseStamped> &event, const std::string &topic)
{
    ...
    ROS_DEBUG_STREAM("TOPIC: " << topic.c_str());
    ROS_DEBUG_STREAM("XYZ message: " << msg->pose.position.x << " " << msg->pose.position.y << " " << msg->pose.position.z);
    ROS_DEBUG_STREAM("Priority: " << unsigned(EntryMap[topic].data.priority));
    if(isLockHolder(topic))
    {
        EntryMap[topic].last_read = ros::Time::now();

        bev_uav_msgs::UAVWaypoint msgPub;

        msgPub.frame = bev_uav_msgs::UAVWaypoint::FRAME_LOCAL_NED;
        msgPub.type  = bev_uav_msgs::UAVWaypoint::TYPE_POSITION;
...
void Priority_manager::cbGeometryMsgsTwistStampedmsg(const ros::MessageEvent<geometry_msgs::TwistStamped const>& event, const std::string &topic)
{
    ...
    ROS_DEBUG_STREAM("TOPIC: " << topic.c_str());
    ROS_DEBUG_STREAM("XYZ message: " << msg->twist.linear.x << " " << msg->twist.linear.y << " " << msg->twist.linear.z);
    ROS_DEBUG_STREAM("Priority: " << unsigned(EntryMap[topic].data.priority));
    if(isLockHolder(topic))
    {
        EntryMap[topic].last_read = ros::Time::now();
        ...
        msgPub.frame = bev_uav_msgs::UAVWaypoint::FRAME_LOCAL_NED;
        msgPub.type  = bev_uav_msgs::UAVWaypoint::TYPE_VELOCITY_BYPASS;
...
void Priority_manager::createSubscribers(ros::NodeHandle *nh)
{
    ...
    for(EntryMap_::iterator it = EntryMap.begin(); it != EntryMap.end(); ++it) {
        ...
        {
            case bev_uav_msgsHeifuWaypoint:
            {
                std::string topic_name = static_cast<std::string>(it->second.data.topic);
                it->second.subscriber = nh->subscribe<bev_uav_msgs::UAVWaypoint>(topic_name.c_str(), 1, boost::bind(&Priority_manager::cbHeifuMsgsHeifuWaypointmsg, this, _1, topic_name));
                break;
            }
